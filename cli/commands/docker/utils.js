...
const generatePluginBlock = (configs, plugin) => {
  ...
  const conf = {
    image: `${registry}octobots/plugin-${plugin.name}-api:${image_tag}`,
    environment: {
...
const deployDbs = async () => {
  ...
  if (configs.mongobi) {
    dockerComposeConfig.services['mongo-bi-connector'] = {
      image: 'octobots/mongobi-connector:dev',
...
const up = async ({ uis, downloadLocales, fromInstaller }) => {
  ...
  const dockerComposeConfig = {
    ...
    services: {
      coreui: {
        image: `octobots/erxes:${(configs.coreui || {}).image_tag || image_tag}`,
        environment: {
      ...
      'plugin-core-api': {
        image: `octobots/core:${(configs.core || {}).image_tag || image_tag}`,
        environment: {
      ...
      crons: {
        image: `octobots/crons:${image_tag}`,
        environment: {
      ...
      'plugin-workers-api': {
        image: `octobots/workers:${image_tag}`,
        environment: {
  ...
  if (configs.essyncer) {
    ...
    dockerComposeConfig.services.essyncer = {
      image: `octobots/essyncer:${essyncer_tag}`,
      environment: {
        ELASTICSEARCH_URL: `http://${
  ...
  if (dashboard) {
    dockerComposeConfig.services.dashboard = {
      image: `octobots/dashboard:${dashboard.image_tag || image_tag}`,
  ...
  for (const name of serviceNames.split(',')) {
    ...
    if (!noimage) {
      ...
      if (['crons', 'gateway', 'client-portal'].includes(name)) {
        await execCommand(
          `docker service update erxes_${name} --image octobots/${name}:${image_tag}`
        );
        continue;
      }
      ...
      if (['dashboard-api'].includes(name)) {
        await execCommand(
          `docker service update erxes_dashboard --image octobots/dashboard:${image_tag}`
        );
        continue;
      }
      ...
      if (name === 'coreui') {
        await execCommand(
          `docker service update erxes_coreui --image octobots/erxes:${image_tag}`
        );
        continue;
      }
      ...
      if (name === 'widgets') {
        await execCommand(
          `docker service update erxes_widgets --image octobots/widgets:${image_tag}`
        );
        continue;
      }
      ...
      if (name === 'core') {
        await execCommand(
          `docker service update erxes_plugin-core-api --image octobots/core:${image_tag}`
        );
        continue;
      }
      ...
      if (name === 'workers') {
        await execCommand(
          `docker service update erxes_plugin-workers-api --image octobots/workers:${image_tag}`
        );
        continue;
      }
      ...
      if (pluginConfig) {
        ...
        await execCommand(
          `docker service update erxes_plugin-${name}-api --image ${registry}octobots/plugin-${name}-api:${tag} --with-registry-auth`
        );
      } else {
